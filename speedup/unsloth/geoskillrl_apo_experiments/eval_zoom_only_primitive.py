...
SYSTEM_PROMPT_PRIMITIVE = (
    ...
    "If you use zoom, output the exact complete tag structure above; do not "
    "omit <|ref|>, <|/ref|>, <|box|>, or <|/box|>. "
    ...
    "reasoning in <think>...</think>. End with exactly one "
    "<answer>...</answer>. If the whole image is enough, skip zoom. After "
    "zooming, verify the crop before analyzing. Never say 'uncertain'."
)
SYSTEM_PROMPT_DIRECT = (
    "You are an intelligent remote sensing analyst. Answer the question from "
    "the satellite image directly. Do not write reasoning or analysis. Output "
    "exactly one tag in this format and no other text: <answer>...</answer>."
)
VISION_TOKEN = "<|vision_start|><|image_pad|><|vision_end|>"
NO_THINK_ASSISTANT_PREFIX = "<think>\n\n</think>\n\n"
...
BOX_PRIMITIVE_RE = re.compile(
    r"<\|box\|>\s*\[\[(.*?)\]\]\s*<\|/box\|>",
    re.DOTALL,
)
BOX_COMPACT_RE = re.compile(r"<zoom>.*?\[\[(.*?)\]\].*?</zoom>", re.DOTALL)
...
def extract_bbox(text: str) -> Optional[list]:
    ...
    if not m:
        m = BOX_PRIMITIVE_RE.search(text)
    if not m:
        m = BOX_COMPACT_RE.search(text)
    if not m:
        return None
    try:
...
def clean_generation_text(text: str, stop_strings: Optional[list[str]] = None) -> str:
    """Keep primitive zoom tokens but remove decode artifacts from padded generation."""
    if stop_strings:
        best = None
...
class ZoomOnlyDispatcher:
    def __init__(self, model, processor, device="cuda", max_new_tokens: int = 1024,
        ...
        self.direct_answer_only = direct_answer_only
    ...
    def build_turn1_prompt(self, question: str) -> str:
        assistant_prefix = NO_THINK_ASSISTANT_PREFIX if self.disable_thinking else ""
        return (
    ...
    def _decode_generated(self, token_ids, stop_strings: Optional[list[str]] = None) -> str:
        text = self.processor.tokenizer.decode(token_ids, skip_special_tokens=False)
    ...
    def _generate(self, prompt: str, images: list,
        ...
        return self._decode_generated(gen[0], stop_strings=stop_strings)
    ...
    def _generate_batch_turn1(self, prompts: list[str], images: list[Image.Image],
        ...
        gen = gen[:, inputs["input_ids"].shape[1]:]
        return [
            self._decode_generated(row, stop_strings=["</zoom>", "</answer>"])
            for row in gen
        ]

    def _generate_batch(self, prompts: list[str], images: list[list[Image.Image]],
                        max_new_tokens: Optional[int] = None,
                        stop_strings: Optional[list[str]] = None) -> list[str]:
        if not prompts:
            return []
        if len(images) != len(prompts):
            raise ValueError(f"batch image mismatch: prompts={len(prompts)} images={len(images)}")
        for prompt, image_list in zip(prompts, images):
            expected_images = prompt.count(VISION_TOKEN)
            if expected_images != len(image_list):
                raise ValueError(
                    f"vision token/image mismatch: prompt has {expected_images}, images={len(image_list)}"
                )
        inputs = self.processor(
            text=prompts, images=images, return_tensors="pt", padding="longest"
        ).to(self.device)
        ...
        gen = gen[:, inputs["input_ids"].shape[1]:]
        return [
            self._decode_generated(row, stop_strings=stop_strings)
            for row in gen
        ]

    def _prepare_after_turn1(self, image: Image.Image, question: str,
                             global_small: Image.Image, effective_scale: float,
                             turn1_prompt: str, turn1_out: str) -> tuple[Optional[dict], Optional[dict]]:
        trajectory = f"[TURN1]\n{turn1_out}\n"
        actions = []
        bbox_pred = None

        if self.direct_answer_only:
            answer = extract_answer(turn1_out)
            return {
                "actions": [], "bbox_pred": None, "answer": answer,
                "trajectory": trajectory, "grammar_ok": answer is not None,
                "error": None if answer else "answer_parse_fail",
            }, None
        ...
        # Turn 2: global + crop for verification + answer.
        turn1_trunc = truncate_after(turn1_out, "</zoom>")
        turn2_prefix = "\n" + (
            NO_THINK_ASSISTANT_PREFIX if self.disable_thinking else "<think>"
        )
        turn2_prompt = (
            turn1_prompt
            + turn1_trunc
        ...
        return None, {
            ...
            "actions": actions,
            "bbox_pred": bbox_pred,
            "turn2_prefix": turn2_prefix,
            "turn2_prompt": turn2_prompt,
            "turn2_images": [global_small, crop_small],
        }

    def _finish_turn2(self, pack: dict, turn2_gen: str) -> dict:
        turn2_prefix = pack["turn2_prefix"]
        trajectory = pack["trajectory"]
        turn2_gen = self._generate(
            pack["turn2_prompt"], pack["turn2_images"], max_new_tokens=self.turn2_max_new_tokens,
            stop_strings=["</answer>"],
        ) if turn2_gen is None else turn2_gen
        ...
        return {
            "actions": pack["actions"], "bbox_pred": pack["bbox_pred"], "answer": ans,
    ...
    def _continue_after_turn1(self, image: Image.Image, question: str,
        ...
        result, pack = self._prepare_after_turn1(
            image, question, global_small, effective_scale, turn1_prompt, turn1_out
        )
        if result is not None:
            return result
        return self._finish_turn2(pack, None)
    ...
    def run_batch_turn1(self, prepared: list[dict]) -> list[dict]:
        ...
        turn2_items: list[tuple[int, dict]] = []
        for idx, (p, prompt, output) in enumerate(zip(prepared, prompts, turn1_outputs)):
            result, pack = self._prepare_after_turn1(
            ...
            if result is not None:
                ...
                turn2_items.append((idx, pack))

        if turn2_items:
            try:
                turn2_outputs = self._generate_batch(
                    [pack["turn2_prompt"] for _, pack in turn2_items],
                    [pack["turn2_images"] for _, pack in turn2_items],
                    max_new_tokens=self.turn2_max_new_tokens,
                    stop_strings=["</answer>"],
                )
                for (idx, pack), output in zip(turn2_items, turn2_outputs):
                    results[idx] = self._finish_turn2(pack, output)
            except Exception:
                for idx, pack in turn2_items:
                    results[idx] = self._finish_turn2(pack, None)
...
def summarize(rows: list, tag: str, model_path: str) -> dict:
    ...
    return {
        ...
        "table": {
            ...
            "Crop Hit@0.3": 100 * sum(1 for r in bbox_rows if r.get("iou", 0) >= 0.3) / max(len(bbox_rows), 1),
            "Crop Hit@0.5": 100 * sum(1 for r in bbox_rows if r.get("iou", 0) >= 0.5) / max(len(bbox_rows), 1),
        },
        "extra": {
...
def write_summary(s: dict, out: Path, tag: str):
    ...
    line = (f"table: Global={t['Global']:.2f} Region={t['Region']:.2f} "
            f"Object={t['Object']:.2f} Avg.Acc={t['Avg.Acc']:.2f} "
            f"APO_IoU={t['APO IoU']:.2f} Hit03={t['Crop Hit@0.3']:.2f} "
            f"Hit05={t['Crop Hit@0.5']:.2f} | "
...
def main():
    ...
    parser.add_argument("--sort_by_type", action="store_true",
                        help="Group object/region/global examples to reduce generation padding waste.")
    parser.add_argument("--primitive_grammar", action="store_true",
                        help="Use the unified <zoom><|ref|><|box|></zoom> prompt and parser.")
    parser.add_argument("--disable_thinking", action="store_true",
                        help="Prefill an empty <think></think> block for Qwen3.5-style no-thinking inference.")
    parser.add_argument("--direct_answer_only", action="store_true",
                        help="Use a direct-answer prompt without zoom; intended for raw base-model VQA baseline.")
    ...
    if args.sort_by_type:
        ...
        samples = sorted(
            samples,
            key=lambda x: (order.get(str(x.get("type", "")).lower(), 3), str(x.get("question_id", ""))),
        )
    ...
    if args.direct_answer_only:
        ...
        system_prompt = SYSTEM_PROMPT_PRIMITIVE if args.primitive_grammar else SYSTEM_PROMPT_JSON
    ...
    dispatcher = ZoomOnlyDispatcher(
        ...
        direct_answer_only=args.direct_answer_only,
    )
