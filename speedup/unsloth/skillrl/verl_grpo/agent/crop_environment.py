#!/usr/bin/env python3
"""Legacy-compatible crop helpers for ZoomEarth two-stage rollout."""
...
def find_image(name_or_path: str | Path) -> Path | None:
    ...
    for directory in IMAGE_DIRS:
        path = directory / str(name_or_path)
        if path.exists():
            return path
    return None
...
def cut_image(image: Image.Image, bbox_orig: Iterable[float], min_size: int = 512) -> Image.Image:
    """Replicate the legacy crop/expand semantics used by prior GRPO code."""
    width, height = image.width, image.height
    x1, y1, x2, y2 = [max(0, min(int(v), s)) for v, s in zip(bbox_orig, [width, height, width, height], strict=True)]
