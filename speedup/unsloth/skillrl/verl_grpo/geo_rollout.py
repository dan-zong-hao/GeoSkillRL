#!/usr/bin/env python3
"""Prompt, image, and rollout helpers for corrected ZoomEarth GRPO."""
...
from geo_rewards import bbox_1024_to_orig  # noqa: E402
...
def prepare_sample(row: dict[str, Any], skillbank: RuntimeSkillBank, *, bbox_coord_mode: str = "xy") -> dict[str, Any]:
...
def build_stage2_context(sample: dict[str, Any], turn1_text: str) -> dict[str, Any] | None:
    ...
    if pred_bbox is None:
        return None
    bbox_orig = bbox_1024_to_orig(
        pred_bbox,
        sample["image_size"],
        coord_mode=str(sample.get("bbox_coord_mode") or "xy"),
    )
