...
SKILLRL_DIR = VQA_ROOT / "speedup" / "unsloth" / "skillrl"
if str(SKILLRL_DIR) not in sys.path:
    sys.path.insert(0, str(SKILLRL_DIR))
from skillbank import format_skill_block, load_skillbank, retrieve_skills
...
class ZoomOnlyDispatcher:
    def __init__(self, model, processor, device="cuda", max_new_tokens: int = 1024,
        ...
        self.skillbank = load_skillbank(skillbank_path) if skillbank_path else load_skillbank(None)
    ...
    def build_turn1_prompt(self, question: str, label: str = "") -> str:
        skills = retrieve_skills(question, self.skillbank, label=label, include_general=False, max_task=2, max_mistakes=0)
    ...
    def _continue_after_turn1(self, image: Image.Image, question: str,
        ...
        if "<answer>" in turn1_out and (
            ...
            return {
                ...
                "trajectory": trajectory, "grammar_ok": True, "repeated_zoom": False, "error": None,
            }
        ...
        try:
            ...
            return {
                ...
                "grammar_ok": False, "repeated_zoom": False, "error": f"cut_fail:{exc}",
            }

        # Turn 2: continue in the same format used by skill-SFT training:
        # </zoom> + crop vision token + <think>[Crop Verification] ... <answer>...
        ...
        turn2_prompt = (
            ...
            + "\n"
            + VISION_TOKEN
            + "\nThe zoom has already been used. Do not output another <zoom>. "
              "Verify whether the crop contains the target, then answer.\n"
            + turn2_prefix
        )
        ...
        repeated_zoom = "<zoom>" in turn2_gen or "</zoom>" in turn2_gen
        trajectory += f"[TURN2]\n{turn2_out}\n"
    ...
    def run_preprocessed(self, image: Image.Image, question: str,
                         global_small: Image.Image, effective_scale: float,
                         label: str = "") -> dict:
        turn1_prompt = self.build_turn1_prompt(question, label)
    ...
    def run_batch_turn1(self, prepared: list[dict]) -> list[dict]:
        if not prepared:
            return []
        prompts = [self.build_turn1_prompt(p["question"], p.get("label", "")) for p in prepared]
        try:
            ...
            return [
                self.run_preprocessed(
                    p["image"], p["question"], p["global_small"], p["scale"], p.get("label", "")
                )
                for p in prepared
            ]
        return [
...
def main():
    ...
    parser.add_argument("--skillbank", type=str, default=str(VQA_ROOT / "json_data" / "zoom_json" / "skillrl" / "skillbank_round0.json"))
        ...
        def row_from_result(pack: dict, result: dict) -> dict:
            ...
            return {
                **base,
                ...
                "repeated_zoom": result.get("repeated_zoom", False),
