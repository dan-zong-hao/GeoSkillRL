...
class BBoxGRPOConfig:
    model_path: str = str(VQA_ROOT / "speedup" / "unsloth" / "sft" / "only_zoom" / "output" / "final_hf")
    train_jsonl: str = str(VQA_ROOT / "json_data" / "zoom_json" / "skillrl" / "bbox_rl_round0.jsonl")
    output_dir: str = str(RL_BBOX_ROOT / "output" / "bbox_grpo_round0")
    ...
    skillbank_path: str = str(VQA_ROOT / "json_data" / "zoom_json" / "skillrl" / "skillbank_round0.json")
    split_manifest: str = ""
...
def rollout_batch(batch: list[dict], sampler: BBoxSampler, cfg: BBoxGRPOConfig):
    ...
    try:
        for sample in batch:
            ...
            skills = retrieve_skills(sample["question"], sampler.skillbank, label=sample.get("label", ""), include_general=False, max_task=2, max_mistakes=0) if sampler.skillbank else []
...
def parse_args():
    ...
    p.add_argument("--model_path", default=str(VQA_ROOT / "speedup" / "unsloth" / "sft" / "only_zoom" / "output" / "final_hf"))
