...
EVAL_DIR="$ROOT/speedup/unsloth/eval_no_seg"
OUT=${OUT:-"$EVAL_DIR/results"}
LOG_DIR="$OUT/logs"
TEST_JSONL="$ROOT/json_data/test-00000-of-00001.jsonl"
MODEL=${MODEL:-"$ROOT/speedup/unsloth/sft/only_zoom/output/final_hf"}
TAG=${TAG:-zoom_only_fast_4gpu}
...
echo "=== Zoom-Only 4-GPU eval start $(date '+%F %T %Z') ==="
echo "model=$MODEL"
...
for gpu in "${GPU_LIST[@]}"; do
  for worker in $(seq 0 $((WORKERS_PER_GPU - 1))); do
    ...
    CUDA_VISIBLE_DEVICES="$gpu" HF_HUB_OFFLINE=1 TOKENIZERS_PARALLELISM=false "$PY" eval_zoom_only.py \
      --tag "$part_tag" \
      --model "$MODEL" \
...
if [ "$status" -ne 0 ]; then
  echo "=== Zoom-Only 4-GPU eval FAILED $(date '+%F %T %Z') ==="
  exit "$status"
fi
...
echo "=== Zoom-Only 4-GPU eval done $(date '+%F %T %Z') ==="
